...
elseif ($action -eq "run") {
    ...
    if (Test-Path $envFilePath) {
        ...
        Get-Content -Path $envFilePath | ForEach-Object {
            ...
            [System.Environment]::SetEnvironmentVariable("AZURE_OPENAI_API_KEY", $env:AZURE_OPENAI_API_KEY)
            ...
            [System.Environment]::SetEnvironmentVariable("AZURE_OPENAI_ENDPOINT", $env:AZURE_OPENAI_ENDPOINT)
            # [System.Environment]::SetEnvironmentVariable("AZURE_OPENAI_DEPLOYMENT_NAME", $env:SERVICE_REGION)
            [System.Environment]::SetEnvironmentVariable("AZURE_OPENAI_DEPLOYMENT_NAME", $env:AZURE_OPENAI_DEPLOYMENT_NAME)
        }
    ...
    Start-Process "python" -ArgumentList "-m", "flask", "run", "-h", "0.0.0.0", "-p", "5000", "--debug"
